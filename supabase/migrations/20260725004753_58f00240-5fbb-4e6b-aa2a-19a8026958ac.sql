...
BEGIN
  ...
  IF v_down_payment > 0 THEN
    INSERT INTO public.financial_transactions(company_id,type,description,amount,transaction_date,due_date,paid_at,status,source,reference_id,reference_number,created_by)
    VALUES(v_company_id,'income','Entrada crediário venda '||COALESCE(v_sale.number,v_sale.id::text),v_down_payment,CURRENT_DATE,CURRENT_DATE,now(),'paid','credit_payment',v_sale_id,v_sale.number,auth.uid()) RETURNING id INTO v_ft_id;
    ...
    VALUES(v_account_id,v_company_id,v_down_payment,COALESCE(v_down_method,'cash'),now(),'Entrada da venda',v_ft_id,'down_payment',v_client_request_id,auth.uid()) RETURNING id INTO v_payment_id;
