CREATE OR REPLACE FUNCTION public.complete_settlement_data(
...
BEGIN
  ...
  -- (evita contabilizar duas vezes lançamentos que já afetaram uma conta).
  v_needs_booking := v_tx.account_id IS NULL;
...
REVOKE ALL ON FUNCTION public.complete_settlement_data(uuid, text, uuid, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.complete_settlement_data(uuid, text, uuid, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.complete_settlement_data(uuid, text, uuid, text) TO service_role;