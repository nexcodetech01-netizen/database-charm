-- Corrige erro "function public.settle_financial_transaction(...) is not
-- unique" ao abrir uma venda no Crediário com valor de entrada (down
-- payment) > 0.
--
-- CAUSA RAIZ: várias migrations foram adicionando parâmetros novos a
-- settle_financial_transaction usando CREATE OR REPLACE FUNCTION. Isso
-- só SUBSTITUI a função existente quando a lista de TIPOS dos
-- parâmetros é idêntica — quando muda (ex.: a migration
-- 20260730133117 acrescentou o 6º parâmetro `_settled_amount`), o
-- Postgres não substitui nada: cria uma função NOVA, e a versão
-- antiga (5 parâmetros) continua existindo ao lado da nova.
--
-- A migration 20260730143216 tentou apagar essa versão antiga de 5
-- parâmetros, mas o banco em produção ainda tinha as duas versões
-- coexistindo — provavelmente por uma divergência entre o histórico
-- de migrations deste repositório e o estado real do banco (o mesmo
-- tipo de problema já visto na auditoria de duplicação de lógica
-- desta semana).
--
-- Com as duas versões vivas, toda chamada com exatamente 5 argumentos
-- fica ambígua pro Postgres (a versão de 6 parâmetros aceita ser
-- chamada com 5, porque o 6º tem valor padrão) — ele não consegue
-- decidir qual delas usar e devolve "is not unique". É exatamente o
-- que create_credit_sale faz ao registrar a entrada do Crediário:
--   PERFORM public.settle_financial_transaction(v_ft_id, ..., NULL, 'Entrada de crediário');
-- — 5 argumentos posicionais, batendo com as duas versões ao mesmo tempo.
--
-- CORREÇÃO: em vez de confiar em qual migration "deveria" ter rodado,
-- descobre dinamicamente TODAS as versões de
-- settle_financial_transaction realmente presentes no banco (via
-- pg_proc) e apaga todas, garantindo que sobre exatamente uma. Depois
-- recria a versão canônica única — cópia exata da lógica mais
-- recente (20260906000000_add_recurring_bills.sql, com o bloco de
-- contas recorrentes), sem nenhuma mudança de comportamento.
