-- HOTFIX: paid_at real da liquidação financeira.
-- Antes: paid_at = (_paid_at::date + 12h) — horário sintético.
-- Agora: timestamptz real informado pelo chamador, ou now().

DROP FUNCTION IF EXISTS public.settle_financial_transaction(uuid, text, uuid, date, text);
...
GRANT EXECUTE ON FUNCTION public.settle_financial_transaction(uuid, text, uuid, timestamptz, text) TO authenticated;

-- Bella Pay: repassar o timestamp real do gateway (sem cast para date).
  ...
  if v_settle and v_charge_id is not null then
    ...
    if v_account_id is null then
      ...
      if v_ft_status = 'paid' then
        ...
        begin
          -- HOTFIX: timestamp real do gateway (ou now()), sem cast para date.
