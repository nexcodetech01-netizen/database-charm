-- Normalização de nome para detecção de duplicidade
CREATE OR REPLACE FUNCTION public.product_name_key(_name text)
RETURNS text
LANGUAGE sql
IMMUTABLE
...
AS $$
  SELECT NULLIF(
    btrim(
      regexp_replace(
        lower(public._sku_strip_accents(coalesce(_name, ''))),
        '[^a-z0-9]+', ' ', 'g'
      )
    ), ''
  );
...
-- Pré-visualização dos grupos duplicados (não altera nada)
...
-- Mesclagem efetiva dos duplicados
CREATE OR REPLACE FUNCTION public.merge_duplicate_products(
  _company_id uuid,
  _dry_run boolean DEFAULT true,
  _delete_unused boolean DEFAULT false
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  FOR grp IN SELECT * FROM public.preview_duplicate_products(_company_id) LOOP
    ...
    LOOP
      -- 1) Transfere o saldo pelo motor oficial de estoque
      IF dup.stock <> 0 THEN
      ...
      -- 2) Repontua o histórico para o produto principal
      ...
      -- 3) Inativa (ou remove, se nunca utilizado)
...
REVOKE ALL ON FUNCTION public.preview_duplicate_products(uuid) FROM public, anon;
REVOKE ALL ON FUNCTION public.merge_duplicate_products(uuid, boolean, boolean) FROM public, anon;
GRANT EXECUTE ON FUNCTION public.product_name_key(text) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.preview_duplicate_products(uuid) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.merge_duplicate_products(uuid, boolean, boolean) TO authenticated, service_role;