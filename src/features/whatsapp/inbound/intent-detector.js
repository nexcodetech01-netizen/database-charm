...
export function parseWebsiteCatalogOrder(text) {
    ...
    const isOrder = t.includes("[PEDIDO-CATALOGO]") ||
    ...
    if (t.includes("Entrega em Tupã"))
        deliveryMethod = "tupa";
    else if (t.includes("Envio para outra cidade"))
 ...
 */
export function parseCatalogProductIntent(text) {
    const t = text.trim();
    // Padrão Meta: "SKU: XXX-XXX"
    const skuMatch = t.match(/SKU:\s*([A-Z0-9\-_]+)/i);
    if (skuMatch)
        return { sku: skuMatch[1] };
 ...
 */
export function isDataSubmissionIntent(text) {
    const t = normalize(text ?? "");
...
export function detectPaymentMethod(text) {
    const t = normalize(text ?? "");
    if (/\b(dinheiro|especie|troco)\b/i.test(t))
        return 'money';
    if (/\b(pix|cartao|credito|debito|link)\b/i.test(t))
        return 'pix_card';
    return null;
}
