...
const INTEREST_RE =
  /\b(gostei|amei|adorei|perfeit[oa]|essa mesmo|esse mesmo|vou levar|vou querer|quero ess[ae]|pode adicionar|fechado|topo)\b/;

/**
 * Confirmação de interesse / adição ao pedido.
 * Reutiliza o parser do carrinho — nenhuma intenção nova é criada.
 */
export function isUpsellTriggerIntent(text: string): boolean {
  const t = normalize(text);
  if (!t) return false;
  return hasAddIntent(t) || INTEREST_RE.test(t);
}

export interface UpsellItem extends ProductSearchItem {
  /** Ativo/inativo — somente ativos são sugeridos. */
  status?: string | null;
  /** Coleções a que o produto pertence (`product_collection_items`). */
  collectionIds?: readonly string[] | null;
  /** Relacionamento explícito cadastrado (quando existir). */
  relatedProductIds?: readonly string[] | null;
  coverImagePath?: string | null;
}

export interface RankUpsellOptions {
  current: UpsellItem;
  candidates: readonly UpsellItem[];
  /** Ids já presentes no carrinho — nunca sugeridos novamente. */
  cartProductIds?: readonly string[];
  /** Categorias complementares configuradas para a categoria atual. */
  complementaryCategoryIds?: readonly string[];
  limit?: number;
}

function isActive(p: UpsellItem): boolean {
 ...
 */
export function rankUpsell(options: RankUpsellOptions): UpsellItem[] {
  ...
  const scored = candidates
    ...
    .map((p) => {
      ...
      if ((p.collectionIds ?? []).some((c) => collections.has(c))) score += 500;
      if (p.categoryId && complementary.has(p.categoryId)) score += 250;
      if (
        current.brand &&
        p.brand &&
...
export const UPSELL_FOOTER =
  "Gostaria de adicionar algum deles ao seu pedido? 😊";

/** Mensagem de sugestões complementares. Vazia quando não há o que sugerir. */
export function formatUpsellMessage(products: readonly UpsellItem[]): string {
