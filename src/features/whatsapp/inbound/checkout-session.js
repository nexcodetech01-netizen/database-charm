 ...
 */
export function formatWebsiteOrderSummary(session, cart) {
    const c = session.customer;
    const items = cart.items.map((i) => `• ${i.name} — ${i.qty} un. — ${money(i.unitPrice)} (Subtotal: ${money(i.subtotal)})`);
    ...
    if (isFreightPending) {
        lines.push("", "📦 A taxa de envio para sua cidade será calculada conforme o CEP e informada antes da confirmação do pedido.");
    }
    lines.push("", `CPF: ${c.cpf ? formatDocument(c) : "-"}`, "Endereço:", formatCustomerAddress(c) || "-", "", `Forma de recebimento: ${session.fulfillment === "pickup" ? "Retirada" : "Entrega"}`, `Forma de pagamento: ${session.payment ? PAYMENT_LABEL[session.payment] : "-"}`, session.payment === "cash"
...
export function formatCheckoutSummary(session, cart) {
    const c = session.customer;
    const items = cart.items.map((i) => `• *${i.name}* (x${i.qty}) — *${money(i.unitPrice)}* (Subtotal: ${money(i.subtotal)})`);
    const lines = [
    ...
    lines.push("", "*Entrega:*", formatFulfillmentLine(session), "", "*Pagamento:*", session.payment ? PAYMENT_LABEL[session.payment] : "-", session.payment === "cash"
        ? `*Troco:* ${session.changeNeeded ? `para ${money(session.changeAmount ?? 0)}` : "Não precisa"}`
        : "", "", "*Itens do pedido:*", ...items, "", `*Subtotal: ${money(cart.total)}*`, `*Frete: ${session.deliveryFee === null ? "Será calculado para envio" : money(session.deliveryFee)}*`, `*Total: ${session.deliveryFee === null ? "A calcular" : money(cart.total + session.deliveryFee)}*`, "", SUMMARY_CONFIRM_MESSAGE);
    return lines.join("\n");
}
...
export async function advanceCheckout(args) {
    ...
    switch (session.step) {
        ...
        case "payment": {
            const payment = parsePayment(text);
            if (!payment)
                return { session, text: PROMPTS.payment, aborted: false };
            const updated = next(session, { payment }, now);
            if (payment === "cash") {
                return {
        ...
        case "WAITING_CUSTOMER_NAME": {
            if (!text)
                return { session, text: `Perfeito! 😊 Qual é o seu nome completo?`, aborted: false };
            const updated = withCustomer(session, { fullName: text }, { step: "WAITING_DOCUMENT" }, now);
            return {
                session: updated,
        ...
        case "buyer_name": {
            ...
            return {
                session: withCustomer(session, { fullName: text }, { step: "person_type" }, now),
        ...
        case "person_type": {
            ...
            if (!personType)
                return { session, text: PROMPTS.person_type, aborted: false };
            return {
        ...
        case "WAITING_DOCUMENT": {
            ...
            const updated = withCustomer(session, { cpf: d, personType: "pf" }, { step: "WAITING_ADDRESS" }, now);
            return {
                session: updated,
            ...
            if (zip && args.resolveCep) {
                ...
                if (info) {
                    const updated = withCustomer(session, {
                        ...
                        city: info.city,
                        state: info.state
                    }, { step: session.deliveryFee === null ? "WAITING_SHIPPING_FEE" : "WAITING_CONFIRMATION" }, now);
                    return {
                        session: syncDelivery(updated),
                        text: PROMPTS[updated.step],
                        aborted: false
                    };
                }
            }
        ...
        case "zip_code": {
            ...
            if (!zip)
                return { session, text: INVALID_CEP_MESSAGE, aborted: false };
            const found = args.resolveCep ? await args.resolveCep(zip) : null;
            if (!found)
                return { session, text: CEP_NOT_FOUND_MESSAGE, aborted: false };
            const updated = withCustomer(session, {
                zipCode: zip,
        ...
        case "address_number": {
            if (!text)
                return { session, text: PROMPTS.address_number, aborted: false };
            const updated = withCustomer(session, { number: text }, { step: "address_complement" }, now);
            return {
                session: syncDelivery(updated),
        ...
        case "address_complement": {
            ...
            const updated = withCustomer(session, { complement: skip ? null : text }, { step: "fulfillment" }, now);
            return {
                session: syncDelivery(updated),
        ...
        case "fulfillment": {
            ...
            if (!kind)
                return { session, text: PROMPTS.fulfillment, aborted: false };
