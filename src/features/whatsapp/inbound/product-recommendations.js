...
const REJECT_RE = /\b(nao gostei|nao curti|nao quero (esse|essa|este|esta)|tem outr[oa]|tem mais algum|outra opcao|outras opcoes|mostra(r)? outr[oa]|quero outr[oa]|tem outr[oa] parecid[oa]|algo parecido|parecido|semelhante|similar|mais barat[oa]|mais car[oa])\b/;
...
export function isAlternativeRequestIntent(text) {
...
export function parsePriceDirection(text) {
    const t = normalize(text);
    if (!t)
        return null;
    if (CHEAPER_RE.test(t))
        return "cheaper";
    if (PRICIER_RE.test(t))
        return "pricier";
    return null;
}
/**
 ...
 */
export function rankRecommendations(options) {
    ...
    const filtered = candidates.filter((p) => {
        if (!p || p.id === current.id)
            return false;
        const price = Number(p.price) || 0;
        if (direction === "cheaper" && !(price < base))
            return false;
        if (direction === "pricier" && !(price > base))
            return false;
        return true;
    });
    const scored = filtered.map((p) => {
        const price = Number(p.price) || 0;
        let score = 0;
        if (current.categoryId && p.categoryId === current.categoryId)
            score += 100;
        if (current.brand &&
            p.brand &&
...
export const NO_RECOMMENDATIONS_MESSAGE = "No momento não encontrei outro produto parecido, mas posso mostrar outras opções da mesma categoria. O que você acha? 😊";
...
export function formatRecommendationsMessage(products) {
...
export function formatRecommendationCaption(product) {
