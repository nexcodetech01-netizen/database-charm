...
import { saveCheckoutSession, handleCheckoutTurn } from "./checkout-session.server";
...
const phoneVariants = (waId) => [waId, waId.replace("@s.whatsapp.net", "")];
export async function handleWhatsAppInboundPayload({ db, msg, tenant, startedAt }) {
    ...
    if (isCatalogOrderMessage) {
        ...
        const promptKey = freshSession.step;
