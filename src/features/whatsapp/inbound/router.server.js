...
function withContactLock(key, fn) {
    ...
    contactLocks.set(key, next.finally(() => {
        if (contactLocks.get(key) === next)
            contactLocks.delete(key);
    }));
    return next;
}
/**
    ...
    for (const entry of entries) {
        ...
        for (const change of changes) {
            ...
            for (const m of messages) {
                ...
                if (!waMessageId || !from || !phoneNumberId)
                    continue;
                out.push({
                    waMessageId,
                    waContactId: from,
...
function formatBellaResponse(r) {
    const parts = [];
    const greeting = getGreeting();
    parts.push(greeting);
    parts.push(""); // Linha em branco após saudação
    if (r.title && r.title !== "Ação executada")
        parts.push(`*${r.title}*`);
    if (r.description)
    ...
    if (r.metrics?.length) {
        for (const m of r.metrics.slice(0, 6)) {
            ...
            if (label && value !== undefined)
                parts.push(`• ${String(label)}: ${String(value)}`);
        }
    }
...
function parseStatusEvents(payload) {
    const out = [];
    const entries = (payload?.entry ?? []);
    for (const entry of entries) {
        const changes = (entry?.changes ?? []);
...
async function processStatusEvents(db, events) {
        ...
        try {
            ...
            const errorText = ev.errorCode != null || ev.errorMessage
                ? [
                    ...
                    .join(" | ")
                : null;
            const updatePayload = { status: ev.status };
            if (errorText)
                updatePayload.error = errorText;
            let updateQuery = db
...
export async function handleWhatsAppInboundPayload(payload) {
        ...
        try {
            ...
            if (tenant === undefined) {
                const { data: company } = await db
                    ...
                    .maybeSingle();
                if (company) {
                    tenant = { companyId: company.id, companyName: company.name ?? null };
                }
                else {
                    ...
                    const { data: fallback } = await db
                        ...
                        .maybeSingle();
                    tenant = fallback
                        ? { companyId: fallback.id, companyName: fallback.name ?? null }
                        : null;
                }
            ...
            await withContactLock(lockKey, () => processOneMessage({ db, msg, tenant: resolvedTenant, startedAt: started }));
        }
        catch (err) {
...
async function processOneMessage({ db, msg, tenant, startedAt }) {
    ...
    const savedState = (conversation.bella_state ?? {});
...
result: NOT INTERCEPTED (NO ACTIVE CHECKOUT)`);
    ...
    if (websiteOrder) {
        ...
        if (sent.ok)
            convUpdate.last_outbound_at = new Date().toISOString();
    ...
    const recommendationTurn = await handleRecommendationTurn({
        db,
        storage: (await import("@/integrations/supabase/client.server")).supabaseAdmin.storage,
        companyId: tenant.companyId,
        phone: msg.phone,
        text: msg.text,
        state: savedState.catalog,
    });
    ...
    const stateToSave = {};
    if (snapshot) {
        if (snapshot.lastModule)
            stateToSave.lastModule = snapshot.lastModule;
        if (snapshot.lastAction)
            stateToSave.lastAction = snapshot.lastAction;
        if (snapshot.lastProvider)
            stateToSave.lastProvider = snapshot.lastProvider;
        if (snapshot.pendingSkill)
            stateToSave.pendingSkill = snapshot.pendingSkill;
    }
    bellaConversationManager.clear(engineKey);
    await db
        .from("whatsapp_conversations")
