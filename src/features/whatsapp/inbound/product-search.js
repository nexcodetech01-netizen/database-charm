...
export function stem(word) {
    ...
    if (w.length > 4 && w.endsWith("aes"))
        return `${w.slice(0, -3)}ao`;
    if (w.length > 4 && w.endsWith("eis"))
...
const MAX_RE = /\b(?:ate|no maximo|maximo (?:de )?|menos de|abaixo de|menor que|nao passe de|nao passar de)\s*(?:r\$\s*)?(\d{1,3}(?:\.\d{3})*(?:,\d{1,2})?|\d+(?:[.,]\d{1,2})?)/;
const MIN_RE = /\b(?:acima de|mais de|maior que|a partir de|no minimo|minimo (?:de )?|apartir de)\s*(?:r\$\s*)?(\d{1,3}(?:\.\d{3})*(?:,\d{1,2})?|\d+(?:[.,]\d{1,2})?)/;
...
export function matchCategoryByStem(text, categories) {
...
export function matchBrand(text, brands) {
    const t = normalize(text);
    let best = null;
    for (const brand of brands) {
        ...
        if (hit && (!best || b.length > normalize(best).length))
            best = brand;
    }
    return best;
}
 ...
 */
export function parseProductQuery(message, context = {}) {
...
export function scoreProduct(product, filters) {
    ...
    if (filters.text) {
        ...
        for (const word of words) {
            const w = stem(word);
            if (nameStems.includes(w))
                score += 12;
            else if (name.includes(normalize(word)))
                score += 6;
        }
    }
    let priceBonus = 0;
    ...
    if (score > 0)
        return score + priceBonus;
    return anchored ? 0 : priceBonus;
}
/** Aplica os filtros de preço e ordena por relevância (depois nome). */
export function rankProducts(products, filters, limit = PRODUCT_SEARCH_LIMIT) {
    return products
...
export function describeFilters(filters) {
    ...
    if (filters.priceMin !== null)
        parts.push(`acima de ${money(filters.priceMin)}`);
    if (filters.priceMax !== null)
        parts.push(`até ${money(filters.priceMax)}`);
    return parts.join(" · ");
}
export function formatSearchResultsMessage(filters, products) {
