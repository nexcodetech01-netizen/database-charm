 ...
 */
import { formatCategoriesMessage, formatProductsMessage, isBackIntent, isCatalogIntent, matchCategory, } from "./catalog-nav";
import { handleProductSearchTurn, listActiveProducts } from "./product-search.server";
import { findCartItemIndex, formatAmbiguousAddMessage, formatCartMessage, formatCartUpdatedMessage, formatClearedMessage, formatRemovedMessage, hasAddIntent, isAnaphoricAdd, matchProduct, parseCartCommand, parseQuantity, } from "./cart";
import { addProduct, clearCartSession, removeAt, } from "./cart-session";
import { getCartSession, saveCartSession } from "./cart-session.server";
export async function listCategoriesWithActiveProducts(db, companyId) {
...
export async function listActiveProductsByCategory(db, companyId, categoryId) {
    const { data } = await db
        .from("products")
        ...
        .order("name");
    return (data ?? []).map((p) => ({ id: p.id, name: p.name, price: Number(p.price), unit: p.unit ?? null }));
}
/**
...
export async function handleCatalogTurn(args) {
    ...
    const session = await getCartSession(companyId, phone);
    ...
    if (command && (inCatalog || session.items.length > 0)) {
        ...
        const index = command.ordinal !== null
            ? command.ordinal - 1
            : command.text
