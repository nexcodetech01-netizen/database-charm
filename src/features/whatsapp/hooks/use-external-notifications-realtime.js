 ...
 */
export function useExternalNotificationsRealtime(companyId, settings, settingsLoading) {
    ...
    const emitEvent = (event, isHistorical) => {
        if (event.wa_message_id?.includes("n8n-10")) {
    ...
    useEffect(() => {
        ...
        if (!companyId)
            return;
        const processExternalEvent = (event, isHistorical = false) => {
            if (event.wa_message_id?.includes("n8n-10")) {
                console.log(`[EXT-NOTIF] processing event n8n-10`);
            }
            // Evita duplicidade
            if (processedIds.current.has(event.wa_message_id))
                return;
