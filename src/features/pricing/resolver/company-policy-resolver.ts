 ...
 */
import type { CompanyPolicy, PolicyOverrides, ResolverWarning } from "./types";
...
export function resolveCompanyLayer(policy: CompanyPolicy): CompanyLayer {
  ...
  if (!isNum(d.minMarginPct) || !isNum(d.idealMarginPct) || !isNum(d.premiumMarginPct)) {
    warnings.push({
      code: "MISSING_COMPANY_DEFAULTS",
      message: "Empresa sem defaults de margem completos (min/ideal/premium). Assumindo 0/0/0.",
