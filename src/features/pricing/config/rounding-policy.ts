...
export function createPsychologicalRounding(endings: readonly number[]): RoundingPolicySpec {
...
export function validateRoundingPolicy(value: unknown, path = "roundingPolicy"): DomainIssue[] {
  if (value === undefined || value === null) return [];
  if (typeof value !== "object") {
    return [issue("INVALID_TYPE", path, `${path} deve ser objeto`)];
  }
  const spec = value as { kind?: unknown; endings?: unknown };
  if (typeof spec.kind !== "string" || !VALID_KINDS.includes(spec.kind as RoundingPolicyKind)) {
    return [
      issue("INVALID_ROUNDING_POLICY", `${path}.kind`, `roundingPolicy.kind inválido`, {
  ...
  if (spec.kind === "psychological") {
    if (!Array.isArray(spec.endings) || spec.endings.length === 0) {
      return [
        issue("INVALID_ROUNDING_POLICY", `${path}.endings`, `endings deve ser array não-vazio`),
      ];
    }
