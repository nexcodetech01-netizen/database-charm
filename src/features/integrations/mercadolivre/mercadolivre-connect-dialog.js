...
import { AlertTriangle, CheckCircle2, ExternalLink, Loader2, RefreshCw, ShieldCheck, ShoppingBag, XCircle, } from "lucide-react";
...
import { disconnectMercadoLivre, getMercadoLivreIntegration, saveMercadoLivreCredentials, startMercadoLivreOAuth, } from "@/lib/mercadolivre.functions";
...
function formatError(err) {
    ...
    if (lower.includes("row-level security") || lower.includes("permission denied")) {
        return {
            ...
            description: "Sua conta não tem acesso à empresa selecionada. Verifique se você está na empresa correta.",
...
export function MercadoLivreConnectDialog({ open, onOpenChange, onStatusChange }) {
    ...
    const handleDisconnect = async () => {
        if (!confirm("Desconectar a conta Mercado Livre desta empresa?"))
            return;
        try {
            await disconnectFn();
      ...
      <DialogContent className="flex max-h-[90vh] w-full max-w-lg flex-col gap-0 p-0">
        <DialogHeader className="shrink-0 border-b border-border/60 p-6 pb-4">
          <DialogTitle className="flex items-center gap-2">
            ...
            Mercado Livre
            {descriptor && tone ? (<Badge variant="outline" className={`ml-1 ${tone.badge}`}>
                {descriptor.label}
              </Badge>) : null}
          </DialogTitle>
          <DialogDescription>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {loading ? (<div className="flex items-center justify-center py-8 text-sm text-muted-foreground">
              <Loader2 className="mr-2 h-4 w-4 animate-spin"/> Carregando…
            </div>) : (<div className="space-y-4">
              {descriptor && tone ? (<Alert className={tone.alert}>
                  <StatusIcon className={`h-4 w-4 ${tone.icon}`}/>
                  <AlertTitle>{descriptor.label}</AlertTitle>
                  <AlertDescription className="space-y-1">
                    <p>{descriptor.message}</p>
                    {descriptor.action ? (<p className="text-xs opacity-90">{descriptor.action}</p>) : null}
                    {connected && status ? (<div className="mt-2 space-y-0.5 text-xs opacity-90">
                        {status.mlUserId ? <div>ML User ID: {status.mlUserId}</div> : null}
                        {status.tokenExpiresAt ? (<div>
                            Token expira em:{" "}
              ...
              {inlineError ? (<Alert variant="destructive">
                  ...
                  <AlertDescription className="space-y-1">
                    ...
                    {inlineError.recovery ? (<p className="text-xs opacity-90">{inlineError.recovery}</p>) : null}
                  </AlertDescription>
                </Alert>) : null}
              ...
              <div className="space-y-3">
                <div className="space-y-1.5">
                  ...
                  <Input id="ml-client-id" value={clientId} onChange={(e) => setClientId(e.target.value)} placeholder="Ex: 1234567890123456" autoComplete="off"/>
                </div>
                <div className="space-y-1.5">
                  <Label htmlFor="ml-client-secret" className="text-xs font-medium">
                    Client Secret{" "}
                    {hasCreds ? (<span className="text-muted-foreground">(deixe em branco para manter)</span>) : null}
                  </Label>
                  <Input id="ml-client-secret" type="password" value={clientSecret} onChange={(e) => setClientSecret(e.target.value)} placeholder={hasCreds ? "••••••••••••" : "Cole o Client Secret do app"} autoComplete="off"/>
                </div>
                <div className="rounded-md border border-dashed border-border/60 bg-muted/30 p-3 text-xs text-muted-foreground">
                  ...
                  <a href="https://developers.mercadolivre.com.br/devcenter" target="_blank" rel="noreferrer" className="mt-2 inline-flex items-center gap-1 text-primary hover:underline">
        ...
        <DialogFooter className="shrink-0 flex-col gap-2 border-t border-border/60 bg-background/95 p-6 pt-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex flex-wrap gap-2">
            <Button onClick={handleSave} variant="outline" disabled={loading || saving || !clientId.trim() || (!clientSecret.trim() && !hasCreds)}>
              {saving ? <Loader2 className="mr-1.5 h-4 w-4 animate-spin"/> : null}
              Salvar credenciais
            </Button>
            <Button onClick={handleAuthorize} disabled={loading || authorizing || !hasCreds} title={!hasCreds ? "Salve as credenciais primeiro" : undefined} variant={needsReauth ? "destructive" : "default"}>
              {authorizing ? (<Loader2 className="mr-1.5 h-4 w-4 animate-spin"/>) : needsReauth ? (<RefreshCw className="mr-1.5 h-4 w-4"/>) : (<ExternalLink className="mr-1.5 h-4 w-4"/>)}
              {needsReauth ? "Reautorizar conta" : "Autorizar conta"}
            </Button>
          </div>
          <div className="flex gap-2">
            {connected ? (<Button variant="ghost" onClick={handleDisconnect} className="text-destructive">
                Desconectar
              </Button>) : null}
