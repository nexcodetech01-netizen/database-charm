...
import { useCategories, useSuppliers, useCreateProduct, useUpdateProduct, productsKeys, useProductImages, useSignedImageUrls, useCreateCategory, } from "../../hooks/use-products";
...
function toState(p) {
    ...
    return {
        ...
        use_category_margin: p.use_category_margin ?? false,
        channel_fee_pct: String(p.channel_pricing_settings?.ml?.fee_pct ?? 0),
        channel_fixed_fee: String(p.channel_pricing_settings?.ml?.fixed_fee ?? 0),
        tax_pct: String(p.channel_pricing_settings?.ml?.tax_pct ?? 0),
...
function calculateKitStock(composition) {
...
export function ProductForm({ companyId, product, duplicateOf, initialPrice }) {
    ...
    useEffect(() => {
        ...
        if (form.product_type === "kit" && productComposition) {
            ...
            setForm((s) => (s.stock === String(calculated) ? s : { ...s, stock: String(calculated) }));
        }
        else {
            setForm((s) => (s.stock === String(productStock) ? s : { ...s, stock: String(productStock) }));
        }
    ...
    const categoryMargin = currentCategory?.target_margin_pct ?? null;
    ...
    const handleFiscalApply = useCallback((v) => setForm((s) => ({ ...s, ...v })), [setForm]);
    ...
    const handleFiscalAutofill = useCallback(() => {
        if (fiscal.categorySuggestion?.ncm) {
            fiscal.applySuggestion({ ncm: fiscal.categorySuggestion.ncm, cest: fiscal.categorySuggestion.cest }, "category");
            return;
        }
    ...
    const submit = async () => {
        ...
        if (!validation.success) {
            ...
            if (["name", "category_id"].includes(field))
                setTab("geral");
            else if (["sku", "barcode", "ncm"].includes(field))
                setTab("estoque");
        ...
        try {
            ...
            if (isEdit) {
                delete finalPayload.stock;
            }
            const saved = isEdit
                ? await updateProduct.mutateAsync({ id: product.id, input: finalPayload })
                : await createProduct.mutateAsync({ ...finalPayload, company_id: companyId });
            ...
            if (!isEdit && saved?.id)
                setCreatedProduct({ id: saved.id, name: payload.name });
            else
    ...
    return (<div className="space-y-8 max-w-5xl mx-auto pb-20">
      ...
      <Tabs value={tab} onValueChange={setTab} className="space-y-8">
        <TabsList className="w-full justify-start h-auto p-1 bg-muted/50 rounded-xl">
          ...
          {form.product_type === 'kit' && (<TabsTrigger value="composicao" className="px-6 py-2.5 rounded-lg">Composição do Kit</TabsTrigger>)}
        </TabsList>

        <div className="bg-card rounded-2xl border shadow-sm p-8 min-h-[500px]">
          <TabsContent value="geral" className="mt-0 space-y-8">
            <GeneralInfoForm form={form} setForm={setForm} categories={categories} suppliers={suppliers} errors={formErrors} onOpenQuickCategory={() => setCategoryDialogOpen(true)} onTitleBlur={() => {
            const formatted = toTitleCasePtBr(form.name);
            if (formatted !== form.name)
          ...
          <TabsContent value="estoque" className="mt-0 space-y-8">
            <div className="grid gap-12">
              <LogisticsForm form={form} setForm={setForm} skuGenerating={skuGenerating} onRegenerateSku={handleRegenerateSku} eanLoading={eanLoading} onEanLookup={handleEanLookup} errors={formErrors}/>
              <div className="h-px bg-border"/>
              <StockForm form={form} setForm={setForm} isEdit={isEdit} onOpenMovement={(t) => { setMovementType(t); setMovementOpen(true); }}/>
              <div className="h-px bg-border"/>
              <FiscalForm form={form} setForm={setForm} onFiscalAutofill={handleFiscalAutofill} fiscalLoading={fiscal.historyLoading || fiscal.masterLoading} errors={formErrors} categoryName={categoryName}/>
            </div>
          </TabsContent>

          <TabsContent value="custos" className="mt-0 space-y-8">
            <div className="space-y-6">
              <PricingForm form={form} setForm={setForm} categoryName={categoryName} categoryMargin={categoryMargin} errors={formErrors} onOpenQuickCategory={() => setCategoryDialogOpen(true)} onApplyCategoryMargin={applyCategoryMargin} onFetchLastPurchase={handleFetchLastPurchase} operationalDefaults={operationalDefaults}/>
            </div>
          </TabsContent>

          <TabsContent value="marketing" className="mt-0 space-y-8">
            <MarketingForm form={form} setForm={setForm} tagInput={tagInput} setTagInput={setTagInput} onAddTag={() => { }} onRemoveTag={() => { }} suggestedTags={suggestedTags} suggestingTags={suggestingTags} onSuggestTags={handleSuggestTags}/>
          </TabsContent>

          <TabsContent value="multimidia" className="mt-0 space-y-8">
            <MultimediaForm companyId={companyId} productId={product?.id} form={form} setForm={setForm} mainImageFile={mainImageFile} setMainImageFile={setMainImageFile} uploadingMainImage={uploadingMainImage} currentMainImageUrl={currentMainImageUrl || ""} uploadingVideo={uploadingVideo} onVideoUpload={handleVideoUpload}/>
          </TabsContent>

          {form.product_type === 'kit' && (<TabsContent value="composicao" className="mt-0 space-y-8">
              <KitCompositionModule companyId={companyId} currentProductId={product?.id} composition={form.composition} setComposition={(composition) => setForm(s => ({ ...s, composition }))}/>
            </TabsContent>)}
        </div>
      </Tabs>

      <MovementFormDialog open={movementOpen} onOpenChange={setMovementOpen} companyId={companyId} defaultProductId={product?.id || ""} defaultType={movementType} onCompleted={() => qc.invalidateQueries({ queryKey: productsKeys.all })}/>
      <SupplierQuickFormDialog companyId={companyId} open={supplierDialogOpen} onOpenChange={setSupplierDialogOpen} onCreated={(s) => setForm(prev => ({ ...prev, supplier_id: s.id }))}/>
      {createdProduct && (<ProductCreatedDialog open={!!createdProduct} onOpenChange={v => !v && setCreatedProduct(null)} productId={createdProduct.id} productName={createdProduct.name} onCreateAnother={() => {
                setCreatedProduct(null);
                setForm(empty);
                setTab("geral");
                navigate({ to: "/produtos/novo" });
            }}/>)}
      <CategoryQuickFormDialog open={categoryDialogOpen} onOpenChange={setCategoryDialogOpen} onCreated={(c) => setForm(prev => ({ ...prev, category_id: c.id }))} onCreate={(name) => createCategory.mutateAsync({ name })} isPending={createCategory.isPending}/>
    </div>);
}
