...
import { computeOfficialPricing, resolveChannelFee, solvePriceForTargetProfit, } from "@/features/pricing/official";
import { ErrorBoundary } from "react-error-boundary";
/** Comissão clássica do Mercado Livre (canal, não taxa de recebimento). */
import { useMutation, useQuery, useQueryClient } from "@tanstack/react-query";
import { useServerFn } from "@tanstack/react-start";
import { Check, Copy, Loader2, Search, ShoppingBag, ExternalLink, Sparkles, RefreshCw, X, ArrowLeft, ArrowRight, AlertTriangle, Wand2, } from "lucide-react";
...
import { publishProductToMercadoLivre, predictMercadoLivreCategory, } from "@/lib/mercadolivre-publish.functions";
...
export function PublishToMercadoLivreDialog({ product, open, onOpenChange }) {
    ...
    return (<Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="fixed left-[50%] top-[50%] z-50 grid w-full max-w-3xl translate-x-[-50%] translate-y-[-50%] gap-4 border bg-background p-0 shadow-lg duration-200 max-h-[90vh] h-auto md:h-[85vh] flex flex-col overflow-hidden">
        <ErrorBoundary key={key} fallbackRender={({ error }) => (<div className="flex-1 flex flex-col items-center justify-center p-8 space-y-4 text-center">
              ...
              <div className="max-w-md w-full">
                ...
                <div className="mt-6 p-4 bg-slate-900 border border-slate-800 rounded-md text-left overflow-auto max-h-[300px]">
                  ...
                  {error?.stack && (<pre className="mt-2 text-[10px] text-slate-500 font-mono leading-tight whitespace-pre-wrap">
                      {String(error.stack)}
                    </pre>)}
                </div>
              </div>
              <Button variant="outline" onClick={() => onOpenChange(false)} className="border-slate-700 text-slate-300 hover:bg-slate-900">
                Fechar Diálogo
              </Button>
            </div>)}>
...
function PublishToMercadoLivreDialogContent({ product, open, onOpenChange }) {
    ...
    const isExpired = integrationQuery.data?.status === "expired" ||
    ...
    const pricingQuery = useQuery({
        ...
        queryFn: () => getProductPricingIntelligence({
    ...
    const channelSettingsQuery = useQuery({
        ...
        queryFn: () => getProductChannelSettings({
            data: { companyId: product?.company_id, productId: product?.id },
        }),
        enabled: open && Boolean(product?.company_id && product?.id),
        staleTime: 60000,
    ...
    const mlSuggestedPrice = useMemo(() => {
        ...
        const marginPct = typeof mlChan?.marginPct === "number" && mlChan.marginPct >= 0
            ? mlChan.marginPct
            : targetMarginPct;
        const feePct = channelFee.feePct;
        let raw = 0;
        if (strategy === "keep_store_profit") {
            if (!(currentStorePrice > 0) || currentStorePrice <= costTotal)
                return null;
    ...
    const [walletTarget, setWalletTarget] = useState(rawProductPrice > 0 ? rawProductPrice.toString() : "");
    const [price, setPrice] = useState(rawProductPrice);
    ...
    const [quantity, setQuantity] = useState(Math.max(1, Math.floor(Number(product?.stock || 0))));
    ...
    const [brand, setBrand] = useState("Generica");
    ...
    const [selectedPhotoPaths, setSelectedPhotoPaths] = useState([]);
    const fileInputRef = useRef(null);
    const videoInputRef = useRef(null);
    const [uploadingSlot, setUploadingSlot] = useState(null);
    const [isUploadingVideo, setIsUploadingVideo] = useState(false);
    const [localImageUrls, setLocalImageUrls] = useState(new Map());
    const [imgErrorMap, setImgErrorMap] = useState(new Map());
    ...
    const photoPaths = useMemo(() => (photosQuery.data ?? [])
        .map((img) => img.path)
        .filter((p) => !!p), [photosQuery.data]);
    ...
    const photoUrlByPath = useMemo(() => {
        const map = new Map();
        const isInvalid = (u) => typeof u === 'string' && (u.toLowerCase().startsWith('failed') ||
    ...
    const extraAttributes = useMemo(() => {
        const list = [];
        const push = (id, value, fallback) => {
            const v = value.trim() || fallback;
            if (v)
                list.push({ id, value_name: v });
        };
    ...
    const publish = useMutation({
        ...
        onError: (err) => {
            ...
            toast.error("Erro no Mercado Livre", {
                description: (<div className="mt-2 text-xs font-mono bg-slate-900 p-2 rounded text-slate-100 max-h-[300px] overflow-auto whitespace-pre-wrap">
            {errorMessage}
          </div>),
                duration: 15000,
            });
    ...
    useEffect(() => {
        if (!open || priceTouched || walletTarget !== "")
            return;
    ...
    async function runPredict(query, opts) {
        if (!query.trim())
            return;
    ...
    useEffect(() => {
        ...
        async function autoFillAttributes() {
            ...
            try {
                ...
                const findVal = (id, search) => {
                    const attr = attrs.find((a) => a.id === id);
                    if (!attr || !attr.values)
                        return "";
                    const match = attr.values.find((v) => search.some(s => v.name.toLowerCase().includes(s.toLowerCase())));
                    return match?.name || attr.values[0]?.name || "";
                };
                if (!gender)
                    setGender(findVal("GENDER", ["Feminino", "Mulher", "Femea"]));
                if (!material)
                    setMaterial(findVal("MAIN_MATERIAL", ["Sintético", "Couro", "Lona"]));
                if (!pattern)
                    setPattern(findVal("PATTERN_NAME", ["Liso", "Solido"]));
                if (!ageGroup)
                    setAgeGroup(findVal("AGE_GROUP", ["Adulto", "Adultos"]));
                if (!withZipper)
                    setWithZipper(findVal("WITH_ZIPPER", ["Sim", "Yes"]));
                if (!season)
                    setSeason(findVal("SEASON", ["Permanente", "Toda"]));
                setBrand("Generica");
    ...
    const uploadPhoto = useMutation({
        mutationFn: async ({ file, slotIndex }) => {
            const nextPosition = photosQuery.data?.length ?? 0;
            if (!product?.company_id || !product?.id)
                throw new Error("Dados da empresa ou produto não encontrados");
        ...
        onError: (err) => {
            toast.error("Falha no processamento", { description: err.message });
        },
    ...
    function movePhoto(index, direction) {
        ...
        if (newIndex < 0 || newIndex >= newPaths.length)
            return;
    ...
    const reprocessPhoto = useMutation({
        ...
        onError: (err) => {
            toast.error("Falha ao tratar imagem", { description: err.message });
        },
    });
    ...
    function SortablePhotoItem({ path, index, url, onToggle, }) {
        ...
        return (<div ref={setNodeRef} style={style} className={`group relative aspect-square overflow-hidden rounded-md border-2 transition-all min-w-[100px] min-h-[100px] sm:min-w-0 sm:min-h-0 ${isDragging ? "border-primary opacity-50 scale-105" : "border-primary ring-2 ring-primary/30"} ${isProcessing ? "cursor-wait" : ""}`}>
        {url && !hasError ? (<img src={url} alt="" className="h-full w-full object-cover rounded-lg" loading="lazy" onError={() => {
                    console.error("Erro ao carregar imagem no slot", index + 1, url);
                    setImgErrorMap(prev => new Map(prev).set(path, true));
                }}/>) : (<div className="h-full w-full bg-muted cursor-pointer flex items-center justify-center" onClick={() => openFilePicker(index)}>
        ...
        {isProcessing ? (<div className="absolute inset-0 z-40 flex flex-col items-center justify-center bg-black/70 text-white backdrop-blur-[2px]">
        ...
        {!isProcessing && (<div {...attributes} {...listeners} className="absolute inset-0 z-10 cursor-grab active:cursor-grabbing" title="Arraste para reordenar"/>)}
        ...
        {/* Botão Remover (X) */}
        {!isProcessing && (<button type="button" onClick={(e) => {
                    e.stopPropagation();
                    onToggle(path);
                }} className="absolute right-1 top-1 z-50 flex h-8 w-8 items-center justify-center rounded-full bg-red-500 text-white shadow-lg ring-2 ring-white hover:bg-red-600 active:scale-95 transition-all" title="Remover foto">
            <X className="h-4 w-4 stroke-[3px]"/>
          </button>)}

        {/* Botões de Reordenação e IA Overlay */}
        {!isProcessing && (<div className="absolute inset-x-0 bottom-0 z-30 flex flex-col gap-1 bg-black/60 p-1 opacity-0 transition-opacity group-hover:opacity-100">
          <div className="flex justify-between gap-1">
            ...
            <button type="button" disabled={index === selectedPhotoPaths.length - 1} onClick={(e) => {
                    e.stopPropagation();
    ...
    const generateDesc = useMutation({
        mutationFn: () => {
            if (!title.trim())
                throw new Error("Título é obrigatório para gerar descrição.");
            return generateDescFn({
                data: {
                    ...
                    supplier: product?.supplier || product?.brand_supplier,
                },
            });
        },
        onSuccess: (res) => {
            ...
            if (attrs) {
                if (attrs.product_type)
                    setProductType(attrs.product_type);
                if (attrs.gender)
                    setGender(attrs.gender);
                if (attrs.bag_type)
                    setBagType(attrs.bag_type);
                if (attrs.material)
                ...
                if (attrs.pattern)
                    setPattern(attrs.pattern);
                if (attrs.with_zipper)
                    setWithZipper(attrs.with_zipper);
                if (attrs.age_group)
                    setAgeGroup(attrs.age_group);
                if (attrs.season)
                    setSeason(attrs.season);
            }
      ...
      <DialogHeader className="p-4 border-b border-border bg-slate-950/50 backdrop-blur-sm shrink-0">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pr-8">
          ...
          <div className="flex flex-col items-end gap-2">
            {validation.isReady ? (<Badge variant="outline" className="bg-success/10 text-success border-success/30 gap-1.5 py-1 px-3">
                <Check className="h-3 w-3"/>
                Pronto para Publicar
              </Badge>) : (<Badge variant="outline" className="bg-amber-500/10 text-amber-600 border-amber-500/30 gap-1.5 py-1 px-3">
                <AlertTriangle className="h-3 w-3"/>
                Pendente
              </Badge>)}
            <div className="flex items-center gap-2 w-full max-w-[150px]">
              <div className="flex-1 h-1.5 bg-muted rounded-full overflow-hidden">
                <div className="h-full bg-primary transition-all duration-500" style={{ width: `${(validation.requirements.filter(r => r?.isValid).length / (validation.requirements?.length || 1)) * 100}%` }}/>
              </div>
        ...
        {isExpired ? (<Alert variant="destructive" className="border-destructive/40">
            ...
            <AlertDescription className="flex flex-col gap-3">
              ...
              <Button type="button" variant="outline" className="w-fit border-destructive/40 text-destructive hover:bg-destructive/10" asChild>
        ...
        <div className={`flex-1 overflow-hidden flex flex-col ${isExpired ? "pointer-events-none opacity-50" : ""}`}>
          <Tabs defaultValue="info" value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col overflow-hidden">
              ...
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  <div className="flex -space-x-1">
                    {validation.requirements.map((req, idx) => (<div key={req?.id || idx} className={`h-2 w-6 rounded-full border border-background ${req?.isValid ? 'bg-success' : req?.critical ? 'bg-destructive/40' : 'bg-muted'}`} title={req?.label}/>))}
                  </div>
                ...
                <div className="flex gap-1.5 overflow-hidden">
                  {validation.requirements.filter(r => !r?.isValid && r?.critical).slice(0, 2).map(req => (<Badge key={req?.id} variant="outline" className="text-[9px] py-0 h-4 border-destructive/30 text-destructive bg-destructive/5">
                      Falta {req?.label}
                    </Badge>))}
                </div>
              </div>
              ...
              <TabsContent value="info" className="m-0 space-y-4 p-4 focus-visible:outline-none">
                ...
                <div className="grid gap-2">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <Button type="button" size="sm" variant="ghost" className="h-7 gap-1.5 text-xs" onClick={buildSeoTitle} title="Gera no padrão [Tipo] [Gênero/Estilo] [Modelo] [Cor]">
                      <Wand2 className="h-3.5 w-3.5 text-primary"/> Título otimizado (SEO)
                    </Button>
                  </div>
                  <Input id="ml-title" value={title} maxLength={60} onChange={(e) => setTitle(e.target.value)}/>
                ...
                <div className="grid gap-2 rounded-lg border border-border bg-muted/20 p-3">
                  <div className="flex items-center justify-between gap-2">
                    <Label htmlFor="ml-category-search" className="flex items-center gap-1.5 text-sm font-semibold">
                      ...
                      {autoSuggested && categoryId ? (<span className="inline-flex items-center gap-1 rounded-full bg-primary/10 px-2 py-0.5 text-[10px] font-medium text-primary">
                          <Sparkles className="h-3 w-3"/> Sugerida
                        </span>) : null}
                    </Label>
                    {categoryId ? (<button type="button" className="text-xs text-muted-foreground hover:text-foreground" onClick={() => {
                setCategoryId("");
                setCategoryLabel("");
                setAutoSuggested(false);
                  ...
                  {categoryId ? (<div className="flex items-center justify-between rounded-md border border-primary/30 bg-primary/5 px-3 py-2">
                      <div className="flex items-center gap-2">
                      ...
                      <div className="relative">
                        ...
                        <Input id="ml-category-search" className="pl-9" placeholder="Pesquise por nome (ex.: Bolsas, Mochilas)" value={categorySearch} onChange={(e) => setCategorySearch(e.target.value)}/>
                        {isSuggesting ? (<Loader2 className="absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 animate-spin text-muted-foreground"/>) : null}
                      </div>

                      {suggestions.length > 0 ? (<div className="max-h-52 overflow-y-auto rounded-md border border-border bg-background">
                          {suggestions.map((s) => (<button key={s.categoryId} type="button" onClick={() => {
                        ...
                        setAutoSuggested(false);
                              ...
                              <div className="min-w-0">
                                ...
                                {s.domainName ? (<p className="truncate text-xs text-muted-foreground">{s.domainName}</p>) : null}
                              </div>
                              <span className="shrink-0 text-[11px] text-muted-foreground">
                                {s.categoryId}
                              </span>
                            </button>))}
                        </div>) : !isSuggesting && categorySearch.trim().length >= 3 ? (<p className="text-xs text-muted-foreground">Nenhuma categoria encontrada.</p>) : (<p className="text-xs text-muted-foreground">Digite 3+ caracteres para buscar.</p>)}
                    </>)}
                </div>
            ...
            if (path) {
                return (<SortablePhotoItem key={path} path={path} index={slot} url={url} onToggle={togglePhoto}/>);
            }
            return (<button key={`slot-${slot}`} type="button" onClick={() => openFilePicker(slot)} disabled={uploadPhoto.isPending} className="flex aspect-square min-w-[100px] h-[100px] items-center justify-center rounded-md border-2 border-dashed border-border bg-background/50 text-muted-foreground transition hover:border-primary hover:text-primary disabled:opacity-60">
            ...
            return (<div className="mt-1 grid gap-1.5">
                        ...
                        <div className="flex gap-1.5 overflow-x-auto pb-1">
                          {unselected.map((path) => {
                    const url = photoUrlByPath.get(path);
                    return (<button key={path} type="button" onClick={() => togglePhoto(path)} className="aspect-square w-12 h-12 overflow-hidden rounded border border-border hover:border-primary shrink-0">
                ...
                <div className="grid gap-2 border-t border-border pt-4">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3">
                    <div className="flex-1 space-y-2">
                      <Input id="ml-video" placeholder="Link do YouTube ou selecione um arquivo MP4..." value={videoUrl} onChange={(e) => setVideoUrl(e.target.value)}/>
                      <input ref={videoInputRef} type="file" accept="video/mp4" className="hidden" onChange={async (e) => {
            const file = e.target.files?.[0];
            if (!file)
                return;
            ...
            try {
                ...
                setIsUploadingVideo(false);
                      ...
                      <Button type="button" variant="outline" size="sm" className="w-full text-xs" disabled={isUploadingVideo} onClick={() => videoInputRef.current?.click()}>
                        {isUploadingVideo ? (<><Loader2 className="mr-2 h-3 w-3 animate-spin"/> Enviando...</>) : ("Selecionar arquivo MP4")}
                      </Button>
                    </div>
                    {videoUrl && (<div className="w-full sm:w-32 aspect-video bg-black rounded-md overflow-hidden shrink-0 flex items-center justify-center">
                        {(() => {
                ...
                if (isYouTube) {
                    ...
                    return videoId ? (<img src={`https://img.youtube.com/vi/${videoId}/mqdefault.jpg`} alt="YouTube Preview" className="w-full h-full object-contain"/>) : null;
                }
              ...
              <TabsContent value="price" className="m-0 space-y-4 p-6 focus-visible:outline-none">
                <div className="grid gap-4">
                  {validation.requirements.find(r => r.id === "price_formula")?.isValid === false && (<Alert variant="destructive" className="py-2 bg-destructive/10 border-destructive/20">
                      <AlertTriangle className="h-4 w-4"/>
            ...
            return (<>
                          <button type="button" onClick={() => setListingType("gold_special")} className={`flex flex-col gap-3 p-4 rounded-xl border-2 text-left transition-all relative ${listingType === "gold_special"
                    ...
                    : "border-border hover:border-primary/40"}`}>
                          ...
                          <button type="button" onClick={() => setListingType("gold_pro")} className={`flex flex-col gap-3 p-4 rounded-xl border-2 text-left transition-all relative ${listingType === "gold_pro"
                    ? "border-primary bg-primary/5 ring-2 ring-primary/10 shadow-sm"
                    : "border-border hover:border-primary/40"}`}>
                            <div className="flex justify-between items-center">
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    <div className="grid gap-2 p-3 bg-muted/20 rounded-xl border border-dashed border-border">
                      ...
                      <div className="flex items-center gap-2">
                        <Input id="ml-price" type="number" step="0.01" className={`h-12 font-mono text-xl font-black bg-background border-2 focus-visible:ring-primary ${validation.requirements.find(r => r.id === "price_formula")?.isValid === false
            ...
            setWalletTarget(Math.max(0, calculatedNet).toFixed(2));
                        ...
                        {usingMlSuggested && !priceTouched && (<Badge variant="outline" className="text-[10px] bg-primary/5 text-primary border-primary/20 shrink-0">
                            <Sparkles className="h-3 w-3 mr-1"/> Sugerido
                          </Badge>)}
                        {priceTouched && !usingMlSuggested && (<Button type="button" size="sm" variant="outline" className="h-8 px-2 text-[10px] border-amber-500/30 text-amber-600 hover:bg-amber-50" onClick={() => {
                ...
                if (desired > 0) {
                    ...
                    setPrice(roundedFinal);
                      ...
                      {validation.requirements.find(r => r.id === "price_formula")?.isValid === false && (<p className="text-[10px] text-destructive font-medium animate-in fade-in slide-in-from-top-1">
                          {validation.requirements.find(r => r.id === "price_formula")?.message}
                        </p>)}
                    </div>

                    <div className={`grid gap-2 p-3 rounded-xl border border-dashed transition-colors ${validation.requirements.find(r => r.id === "price_formula")?.isValid === false
            ? "bg-destructive/5 border-destructive/30"
            : "bg-success/5 border-success/30"}`}>
                      <Label className={`text-xs font-semibold uppercase tracking-tight ${validation.requirements.find(r => r.id === "price_formula")?.isValid === false
            ? "text-destructive"
            : "text-success"}`}>
                        Quanto você recebe (Líquido)
                      </Label>
                      <div className={`flex items-center gap-2 h-12 px-3 bg-background/50 rounded-lg border ${validation.requirements.find(r => r.id === "price_formula")?.isValid === false
            ? "border-destructive/20"
            : "border-success/20"}`}>
                        <span className={`font-mono text-xl font-black ${validation.requirements.find(r => r.id === "price_formula")?.isValid === false
            ? "text-destructive"
            : "text-success"}`}>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="grid gap-2">
                      ...
                      <Input id="ml-qty" type="number" min={1} value={quantity} onChange={(e) => setQuantity(Number(e.target.value))}/>
                    </div>
                    <div className="grid gap-2">
                ...
                <div className="grid gap-4">
                  <div className="flex flex-col gap-3">
                    <div className="flex items-center justify-between">
                      ...
                      <Button type="button" size="sm" variant="ghost" className="h-7 text-[10px]" onClick={async () => {
            ...
            setTimeout(() => setDescCopied(false), 2000);
                    ...
                    <Button type="button" variant="default" className="w-full h-12 text-sm font-bold bg-gradient-to-r from-primary to-primary/80 hover:opacity-90 shadow-lg group transition-all" onClick={() => generateDesc.mutate()} disabled={generateDesc.isPending || !title.trim()}>
                  ...
                  {aiPreview && (<div className="rounded-lg border-2 border-primary/30 bg-primary/5 p-4 space-y-3 animate-in fade-in slide-in-from-top-2">
                      <div className="flex items-center justify-between">
                        <h4 className="text-sm font-bold flex items-center gap-2 text-primary">
                          <Sparkles className="h-4 w-4"/> Prévia da IA
                        </h4>
                        <div className="flex gap-2">
                          <Button type="button" size="sm" variant="outline" className="h-7 text-[10px]" onClick={() => setAiPreview(null)}>
                            Descartar
                          </Button>
                          <Button type="button" size="sm" className="h-7 text-[10px] bg-primary text-primary-foreground" onClick={() => {
                ...
                toast.success("Aplicado com sucesso!");
                  ...
                  <Textarea id="ml-desc" value={description} onChange={(e) => setDescription(e.target.value)} rows={12} className="font-sans text-sm leading-relaxed resize-none focus-visible:ring-primary/30" placeholder="A descrição profissional aparecerá aqui..."/>
        ...
        <DialogFooter className="px-4 py-4 border-t border-border bg-slate-950/50 backdrop-blur-sm shrink-0">
          <div className="flex flex-col sm:flex-row gap-3 w-full sm:justify-end">
            <Button variant="outline" onClick={() => onOpenChange(false)} disabled={publish.isPending} className="w-full sm:w-auto h-11">
              Cancelar
            </Button>
            <Button onClick={() => {
            ...
            publish.mutate();
        }} disabled={!canPublish} className="w-full sm:w-auto h-11 px-8 font-bold">
              {publish.isPending ? (<>
