...
import { fiscalSuggestionService, normalizeCest, normalizeNcm, } from "../lib/fiscal-suggestions";
import { ncmMasterService } from "../lib/ncm-master";
export function useFiscalAutofill({ companyId, name, categoryId, material, categories, ncm, cest, onApply, }) {
    const [source, setSource] = useState(ncm ? "manual" : "category");
    ...
    useEffect(() => {
        async function checkMasterNcm() {
            if (!category)
                return;
            if (lastCategoryRef.current === categoryId && lastMaterialRef.current === material)
                return;
            ...
            try {
                ...
                if (!canOverwrite)
                    return;
                if (ncm === targetNcm && cest === targetCest)
                    return;
                setSource("category");
                onApply({ ncm: targetNcm, cest: targetCest });
                if (masterSuggestion) {
    ...
    const applySuggestion = useCallback((values, nextSource) => {
