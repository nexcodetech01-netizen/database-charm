...
export function normalizeCest(value) {
    const digits = (value ?? "").replace(/\D/g, "");
...
export function isValidCest(value) {
...
export const fiscalSuggestionService = {
     ...
     */
    async byName(companyId, name, limit = 3) {
        const term = name.trim();
        if (!companyId || term.length < 3)
            return [];
    ...
    async byBarcode(companyId, barcode) {
        const code = barcode.trim();
        if (!companyId || code.length < 8)
            return null;
