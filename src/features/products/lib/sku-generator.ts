 ...
 */
const GENERIC_MODEL_WORDS = new Set<string>([
  "bolsa", "bolsas",
  "carteira", "carteiras",
  "mochila", "mochilas",
  "mala", "malas",
 ...
 */
const SKIP_CONNECTORS = new Set<string>([
 ...
 */
function findModelIndex(words: string[], start: number, categoryName?: string | null): number {
  const categoryKey = categoryName ? normalizeKey(categoryName) : null;
  for (let i = start; i < words.length; i++) {
    const key = normalizeKey(words[i]);
    if (!key) continue;
    if (SKIP_CONNECTORS.has(key)) continue;
    if (GENERIC_MODEL_WORDS.has(key)) continue;
    if (categoryKey && key === categoryKey) continue;
 ...
 */
export function buildSkuBase(name: string, categoryName?: string | null): string | null {
  ...
  if (modelIdx === -1) {
    ...
    if (!fallback) return categoryPrefix || null;
  ...
  for (let i = words.length - 1; i > modelIdx; i--) {
    const key = normalizeKey(words[i]);
    if (!key || SKIP_CONNECTORS.has(key) || !/[a-z0-9]/.test(key)) continue;
    colorIdx = i;
    break;
  }
