...
export const lookupProductByEan = createServerFn({ method: "POST" })
    ...
    try {
        const json = await integrationFetchJson(`https://world.openfoodfacts.org/api/v2/product/${data.barcode}.json?fields=product_name,product_name_pt,generic_name,brands,quantity,image_front_url,image_url,categories`, { method: "GET", headers: { "User-Agent": "NexOS-ERP/1.0 (product-registration)" } }, { integration: "openfoodfacts", timeoutMs: 8000, maxAttempts: 2 });
        const product = json.product;
        if (json.status !== 1 || !product)
            return empty;
        const name = clean(product.product_name_pt) ??
