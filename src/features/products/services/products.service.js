...
export const productsService = {
    ...
    async create(input) {
        ...
        if (duplicate) {
            const { stock, company_id: _companyId, sku, 
            // Mesmo produto: a descrição já salva no banco nunca é sobrescrita.
            description: _ignoredDescription, price: incomingPrice, cost: incomingCost, ...rest } = input;
            const num = (v) => {
                const n = Number(v);
            ...
            return { ...updated, __merged: true, __matchedBy: duplicate.matchedBy };
        }
        const { composition, stock, ...insertPayload } = input;
        const { data, error } = await supabase
            .from("products")
        ...
        if (composition && composition.length > 0 && data?.id) {
            const components = composition.map((c) => ({
                company_id: input.company_id,
                parent_id: data.id,
    ...
    async update(id, input) {
        ...
        const { stock: _ignoredStock, ...safeInput } = input;
        const { composition, ...safeInputWithoutComp } = safeInput;
        const updated = await updateRow("products", id, safeInputWithoutComp);
        // Sync composition
        if (composition !== undefined) {
            ...
            if (composition.length > 0) {
                ...
                const companyId = updated?.company_id || input?.company_id;
                if (companyId) {
                    const components = composition.map((c) => ({
        ...
        if (touchesMl && rec?.ml_item_id) {
            ...
            void import("@/lib/mercadolivre-sync.functions")
                .then(({ syncProductToMercadoLivre }) => syncProductToMercadoLivre({ data: { productId: id } }).catch(() => {
    ...
    async remove(id) {
        ...
        if (countError)
            throw countError;
        if ((count ?? 0) > 0) {
            throw new Error("Este produto possui movimentação de estoque e não pode ser excluído.");
        }
