...
import { formatSalesMetric } from "./selectors";
import type { BellaSalesDetail, BellaSalesHealth, BellaSalesMetric } from "./types";

export interface BellaSalesSummaryProps {
  metrics: readonly BellaSalesMetric[];
  details: readonly BellaSalesDetail[];
...
/** Resumo comercial da Bella — leitura das métricas já apuradas. */
export function BellaSalesSummary({
  metrics,
  details,
  health,
  loading = false,
  className,
}: BellaSalesSummaryProps) {
  return (
    <div className={cn("space-y-3", className)} data-testid="bella-sales-summary">
          ...
          <ul className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3">
            {metrics.map((metric) => (
              <li
                ...
                data-testid={`bella-sales-metric-${metric.id}`}
              >
                <p className="text-xs text-muted-foreground">{metric.label}</p>
                <p className="text-base font-semibold tabular-nums">
                  {formatSalesMetric(metric)}
                </p>
                {metric.hint ? (
          ...
          <ul className="space-y-1.5 rounded-xl border border-border/60 p-3">
            {details.map((detail) => (
              <li
                ...
                data-testid={`bella-sales-detail-${detail.id}`}
              >
                <a
