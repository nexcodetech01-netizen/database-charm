...
async function decryptSecret(raw) {
 ...
 */
export async function readSecret(companyId, kind, ownerId, environment) {
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    let q = supabaseAdmin.from("fiscal_secrets")
    ...
    const log = (decryptSuccess, err) => console.info("[fiscal] readSecret", {
    ...
    try {
        const plain = await decryptSecret(row.ciphertext);
        log(Boolean(plain));
        return plain;
    }
    catch (err) {
...
export async function readProviderCredentials(companyId, providerId, environment) {
    if (providerId === "mock")
...
export async function readProviderEnvironment(supabase, companyId, environment) {
    const { data } = await anyFrom(supabase, "fiscal_provider_environments")
        .select("api_url, provisioned_at, provisioned_environment, provisioned_certificate_id," +
...
export async function upsertProviderEnvironment(supabase, companyId, environment, patch) {
    const { data } = await anyFrom(supabase, "fiscal_provider_environments")
        .update(patch)
        .eq("company_id", companyId)
        .eq("environment", environment)
...
async function buildContext(supabase, companyId, saleId, environmentOverride, model = "55") {
    ...
    const { data: companyRaw } = await supabase
        .from("companies")
        .select("id, name, trade_name, cnpj, ie, address, address_number, neighborhood, city, state, zip_code, phone")
        .eq("id", companyId)
        .maybeSingle();
    const co = (companyRaw ?? {});
    ...
    const seriesFromSettings = (model === "65" ? st?.nfce_series : st?.nfe_series) ?? 1;
    ...
    const payload = {
        ...
        fiscal: {
            operationNature: st?.operation_nature ?? "Venda de mercadoria adquirida ou recebida de terceiros",
...
function validatePayload(ctx) {
    ...
    if (p.items.length === 0)
        issues.push({ field: "items", message: "Venda sem itens." });
 ...
 */
export async function persistArtifact(provider, companyId, url, objectPath) {
...
async function persistArtifactTracked(args) {
    ...
    await appendEvent(supabase, companyId, documentId, "artifact_failed", {
        kind,
...
async function markArtifactsPending(supabase, companyId, documentId, pending, lastError) {
...
async function registerCertificate(provider, ctx, companyId) {
    ...
    if (!password)
        return "Senha do certificado A1 não cadastrada.";
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const { data: file, error } = await supabaseAdmin.storage
        .from("fiscal-certificates")
        .download(ctx.certificateStoragePath);
    if (error || !file)
        return "Não foi possível ler o certificado A1 armazenado.";
...
export async function readProviderProvisioning(supabase, companyId, environment) {
    ...
    const { data } = await anyFrom(supabase, "fiscal_provider_config")
        .select("provisioned_at, provisioned_environment, provisioned_certificate_id, provisioned_note")
        .eq("company_id", companyId)
        .maybeSingle();
    const row = (data ?? null);
    return {
 ...
 */
export function isProvisionedFor(state, environment, certificateId) {
    if (!state.provisionedAt)
        return false;
    if (state.provisionedEnvironment !== environment)
        return false;
    if (!state.provisionedCertificateId)
        return true; // provisionamento manual
...
export async function clearProviderProvisioning(supabase, companyId, environment) {
    const patch = {
        provisioned_at: null,
...
export async function provisionProviderCertificateEngine(args) {
    ...
    const { data: coRaw } = await supabase
        ...
        .eq("id", companyId)
        .maybeSingle();
    const cnpj = (coRaw ?? null)?.cnpj ?? "";
    if (!cnpj)
        return { ok: false, message: "CNPJ da empresa não cadastrado." };
    if (!cert?.id || !cert.storage_path)
        return { ok: false, message: "Nenhum certificado A1 ativo cadastrado." };
    const error = await registerCertificate(provider, {
...
// ------------------------------------------------------------------ events
async function appendEvent(supabase, companyId, documentId, eventType, payload, actorId) {
...
async function patchDocument(supabase, companyId, documentId, patch) {
 ...
 */
async function allocateNumber(supabase, companyId, documentId, series, model = "55") {
    const { data, error } = await supabase.rpc("fiscal_allocate_nfe_number", {
...
async function releaseNumber(supabase, companyId, documentId) {
    try {
        await supabase.rpc("fiscal_release_nfe_number", {
            _company_id: companyId,
            _document_id: documentId,
        });
    }
 ...
 */
async function findActiveSaleDocument(supabase, companyId, saleId) {
    const { data } = await anyFrom(supabase, "fiscal_documents")
        .select(DOC_COLS)
        .eq("company_id", companyId)
        .eq("sale_id", saleId)
        .in("status", ACTIVE_FISCAL_STATUSES)
        .order("created_at", { ascending: false })
        .limit(1)
        .maybeSingle();
    return data ?? null;
}
export async function issueNfeFromSaleEngine(input) {
    ...
    const { data: created, error: createErr } = await anyFrom(supabase, "fiscal_documents")
        ...
        request_payload: { reference: ctx.payload.reference },
    })
        .select(DOC_COLS)
        .single();
    if (createErr) {
    ...
    await appendEvent(supabase, companyId, documentId, "created", {
        source: "sales",
        saleId,
        provider: provider.id,
        environment: ctx.environment,
    ...
    if (provider.requiresCertificate) {
        ...
        if (alreadyProvisioned) {
            await appendEvent(supabase, companyId, documentId, "certificate_skipped", {
            ...
            await appendEvent(supabase, companyId, documentId, "signed", {
                certificateId: ctx.certificateId,
            }, userId);
        }
    }
    // 4) Reserva transacional do número + transmissão.
    const allocatedNumber = await allocateNumber(supabase, companyId, documentId, ctx.seriesFromSettings, model);
    if (ctx.payload.fiscal)
        ctx.payload.fiscal.number = allocatedNumber;
    await appendEvent(supabase, companyId, documentId, "number_allocated", { number: allocatedNumber, series: ctx.seriesFromSettings, model }, userId);
    await patchDocument(supabase, companyId, documentId, { status: "sending" });
    // Auditoria: payload lógico completo enviado ao provedor.
    await appendEvent(supabase, companyId, documentId, "sent", {
        provider: provider.id,
        environment: ctx.environment,
        apiUrl: ctx.apiUrl,
        requestPayload: ctx.payload,
    ...
    try {
        ...
        await appendEvent(supabase, companyId, documentId, "error", { message, transmissionMs: Date.now() - transmitStart, releasedNumber: allocatedNumber }, userId);
        return errored;
    }
    ...
    if (!result.ok || result.status === "rejected") {
        ...
        await releaseNumber(supabase, companyId, documentId);
        await appendEvent(supabase, companyId, documentId, "rejected", {
    ...
    await appendEvent(supabase, companyId, documentId, result.status === "authorized" ? "authorized" : "sent", {
        protocol: result.protocol ?? null,
...
// ------------------------------------------------------------------ status
async function providerForDocument(supabase, companyId, documentProvider, environment) {
    const { data: cfgRaw } = await anyFrom(supabase, "fiscal_provider_config")
...
export async function refreshDocumentStatusEngine(args) {
    ...
    if (status === "cancelled" || status === "rejected" || status === "draft")
        return doc;
    const environment = doc.environment === "production" ? "production" : "homologation";
    const provider = await providerForDocument(supabase, companyId, doc.provider ?? null, environment);
    const ref = docRef(doc);
    if (!ref.accessKey && !ref.providerRef)
        return doc;
    const result = await provider.getStatus(ref);
    if (!result.ok) {
        await appendEvent(supabase, companyId, documentId, "error", {
    ...
    if (status === "cancelling") {
        ...
        if (result.status === "cancelled" && confirmedProtocol) {
            return await finalizeCancellation({
                ...
                documentId,
                doc,
                reason: doc.cancellation_reason ?? "Cancelamento solicitado.",
        ...
        if (result.status === "authorized") {
            ...
            await appendEvent(supabase, companyId, documentId, "error", { scope: "cancel", message: "Cancelamento não homologado pela SEFAZ." }, userId);
    ...
    if (result.status === status && !result.accessKey)
        return doc;
    const patch = { status: result.status };
    if (result.accessKey)
        patch.access_key = result.accessKey;
    if (result.protocol)
        patch.protocol = result.protocol;
    if (result.number)
        patch.number = result.number;
    if (result.series)
        patch.series = result.series;
    if (result.status === "authorized") {
        patch.protocol_at = doc.protocol_at ?? new Date().toISOString();
        let pending = normalizePendingKinds(doc.artifacts_pending);
        let lastError = doc.artifacts_last_error ?? null;
        const key = result.accessKey ?? doc.access_key ?? documentId;
    ...
    patch.response_payload = (result.raw ?? null);
    const updated = await patchDocument(supabase, companyId, documentId, patch);
    if (result.status !== status) {
        await appendEvent(supabase, companyId, documentId, result.status, {
            source: "status-refresh",
            protocol: result.protocol ?? null,
        }, userId);
    }
...
export async function cancelDocumentEngine(args) {
    ...
    if (status === "cancelling")
        throw new Error("Cancelamento já solicitado — aguardando confirmação da SEFAZ. Use “Atualizar status”.");
    ...
    if (status !== "authorized") {
        ...
        await appendEvent(supabase, companyId, documentId, "cancelled", {
            scope: "local",
            reason,
        }, userId);
    ...
    const ref = docRef(doc);
    if (!ref.accessKey)
        throw new Error("NF-e autorizada sem chave de acesso.");
    ...
    const eligibility = evaluateCancelEligibility({
        ...
        protocolAt: doc.protocol_at ?? null,
        createdAt: doc.created_at ?? null,
    }, new Date());
    if (!eligibility.allowed) {
        await appendEvent(supabase, companyId, documentId, "error", { scope: "cancel", message: eligibility.reason ?? null, blocked: true }, userId);
    ...
    await appendEvent(supabase, companyId, documentId, "cancel_requested", { scope: "sefaz", reason, requestedAt, deadline: eligibility.deadline ?? null }, userId);
    ...
    let result;
    try {
        result = await provider.cancelNfe({ accessKey: ref.accessKey, providerRef: ref.providerRef }, reason);
    }
    catch (err) {
        ...
        await appendEvent(supabase, companyId, documentId, "cancelling", { scope: "sefaz", stage: "transport_error", message, reason }, userId);
        throw new Error("Falha de comunicação com o provedor. O cancelamento continua em processamento — use “Atualizar status”.");
    }
    ...
    if (result.status !== "cancelled" || !result.protocol) {
        ...
        await appendEvent(supabase, companyId, documentId, "cancelling", {
            scope: "sefaz",
...
async function finalizeCancellation(args) {
    const { supabase, provider, companyId, userId, documentId, doc, reason, protocol, cancelledAt, cancellationXmlUrl, raw, deadline, previousStatus, environment, } = args;
    ...
    let cancellationXmlPath = doc.xml_cancellation_path ?? null;
    let pending = normalizePendingKinds(doc.artifacts_pending);
    let lastError = doc.artifacts_last_error ?? null;
    ...
    const cancelled = await patchDocument(supabase, companyId, documentId, {
        ...
        cancelled_by: doc.cancelled_by ?? userId ?? null,
        cancellation_reason: reason.slice(0, 500),
...
export async function reprocessDocumentArtifactsEngine(args) {
    ...
    const docLike = {
        ...
        status: doc.status,
        accessKey: doc.access_key ?? null,
        xmlAuthorizedPath: doc.xml_authorized_path ?? null,
        danfePath: doc.danfe_path ?? null,
        xmlCancellationPath: doc.xml_cancellation_path ?? null,
    ...
    patch.artifacts_checked_at = new Date().toISOString();
    const updated = await patchDocument(supabase, companyId, documentId, patch);
    await appendEvent(supabase, companyId, documentId, stillPending.length === 0 ? "artifact_recovered" : "artifact_pending", {
        attempted: targets,
        recovered,
        stillPending,
        lastError,
