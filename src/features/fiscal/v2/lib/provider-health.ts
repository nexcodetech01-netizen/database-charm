...
export type ProviderHealthStatus = "ok" | "warning" | "error" | "skipped";

export type ProviderHealthItemId =
  | "admin_token"
  | "company_token"
  | "api"
  | "certificate"
...
export interface ProviderProbeFacts {
  httpStatus: number;
  durationMs: number;
  detail?: string;
  networkError?: string;
...
export interface ProviderHealthFacts {
  ...
  /** Probe de `GET /v2/nfe/{ref}` com o Token Empresa. */
  companyProbe?: ProviderProbeFacts | null;
  /** Probe de `GET /v2/empresas` com o Token Principal. */
  adminProbe?: ProviderProbeFacts | null;
}

export const HEALTH_ITEM_LABEL: Record<ProviderHealthItemId, string> = {
...
function judgeToken(
  probe: ProviderProbeFacts,
  opts: { authOkStatuses: number[] },
  ...
  if (probe.httpStatus === 401) {
    return {
      status: "error",
      message:
        "Credencial recusada (HTTP 401). A Focus emite tokens distintos por ambiente — confirme que este token é deste ambiente.",
    };
  }
  if (probe.httpStatus === 403) {
    return {
      status: "error",
      message: `Escopo insuficiente para este token (HTTP 403).${
  ...
  return {
    status: "warning",
    message: `Resposta inesperada (HTTP ${probe.httpStatus}).${
      probe.detail ? ` ${probe.detail}` : ""
    }`,
  };
}

export function buildProviderHealthItems(facts: ProviderHealthFacts): ProviderHealthItem[] {
  ...
  if (isMock) {
    return [
      ...
      item(
        "certificate",
        facts.hasActiveCertificate ? "ok" : "warning",
        facts.hasActiveCertificate
  ...
  if (!facts.hasAdminToken) {
    items.push(
      item(
        "admin_token",
        facts.provisionedAt ? "warning" : "error",
        facts.provisionedAt
  ...
  if (!facts.hasCompanyToken) {
    items.push(
      item("company_token", "error", "Não cadastrado. Sem ele não é possível emitir NF-e."),
    );
  ...
  if (!facts.apiUrl) {
    ...
    const probes = [facts.companyProbe, facts.adminProbe].filter(Boolean) as ProviderProbeFacts[];
    ...
    if (networkError) {
      ...
      items.push(
        item("api", "warning", `Provedor respondeu HTTP ${unavailable.httpStatus} em ${facts.apiUrl}.`),
      );
  ...
  items.push(
    item(
      "certificate",
      facts.hasActiveCertificate ? "ok" : "error",
      facts.hasActiveCertificate
        ? "Certificado A1 ativo."
...
export function summarizeProviderHealth(items: ProviderHealthItem[]): {
