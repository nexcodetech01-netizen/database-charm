...
export function cancellationDeadline(protocolAt, fallback) {
    ...
    if (Number.isNaN(t))
        return null;
    return new Date(t + CANCEL_WINDOW_HOURS * 3600000).toISOString();
}
/** Avalia se a NF-e pode ser cancelada agora. */
export function evaluateCancelEligibility(doc, now = new Date()) {
    ...
    if (hoursLeft <= 0) {
        return {
            ...
            deadline,
            hoursLeft,
            reason: "Prazo legal de 24 horas para cancelamento expirado. Emita uma nota de devolução.",
        };
    }
