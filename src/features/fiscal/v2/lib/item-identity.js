 ...
 */
export function resolveCommercialCode(sku, fallback) {
    const clean = typeof sku === "string" ? sku.trim() : "";
    if (clean.length > 0)
        return clean.slice(0, CODE_MAX_LENGTH);
 ...
 */
export function resolveCommercialUnit(unit) {
    if (typeof unit !== "string")
        return DEFAULT_COMMERCIAL_UNIT;
    const clean = unit
        .normalize("NFD")
        .replace(/[\u0300-\u036f]/g, "")
        .replace(/[^A-Za-z0-9]/g, "")
