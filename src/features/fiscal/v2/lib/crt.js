 ...
 */
export const CRT_VALUES = [1, 2, 3, 4];
 ...
 */
export function requireCrt(crt) {
    if (!isValidCrt(crt))
        throw new Error(CRT_NOT_CONFIGURED_MESSAGE);
    return crt;
}
/**
 ...
 */
export function resolveProviderCrt(crt, enableCrt4Mei) {
    const value = requireCrt(crt);
    if (value === 4 && !enableCrt4Mei)
        return 1;
    return value;
}
