 ...
 */
export function allocateDiscount(amounts, discount) {
    const result = amounts.map(() => 0);
    const total = round2(discount);
    if (!Number.isFinite(total) || total <= 0 || amounts.length === 0)
        return result;
 ...
 */
export function expectedDocumentTotal(totals) {
    return round2((Number(totals.products) || 0) -
        (Number(totals.discount) || 0) +
        (Number(totals.freight) || 0));
}
/** Os totais fecham a equação da NF-e (tolerância de 1 centavo)? */
export function totalsAreConsistent(totals) {
