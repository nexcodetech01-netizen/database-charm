...
export const FISCAL_DOC_PRIORITY = {
    cancelling: 5,
    authorized: 4,
    cancelled: 3,
    sending: 2,
...
export function resolveActiveFiscalDocument(docs) {
 ...
 */
export function resolveFiscalBadgeKey(doc) {
    if (!doc)
        return "none";
    switch (doc.status) {
        case "authorized":
 ...
 */
export function getFiscalStatusBadge(input) {
    const key = typeof input === "string"
        ? (input in FISCAL_BADGE_UI ? input : "none")
 ...
 */
export function blocksNewFiscalDocument(docs) {
