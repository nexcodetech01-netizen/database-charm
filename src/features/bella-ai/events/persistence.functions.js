...
export const readNotification = createServerFn({ method: "POST" })
    ...
    }).refine((d) => !!d.notificationId || !!d.eventType, { message: "Informe notificationId OU eventType (com referenceId)." })
}).parse(data))
    .handler(async ({ data }) => {
    if (data.data.notificationId) {
        return markNotificationAsRead(data.data.notificationId, data.data.companyId);
    }
    return markNotificationAsReadByContent(data.data.companyId, data.data.eventType, data.data.referenceId ?? null);
});
