...
export function EditConsignmentItemsDialog({ open, onOpenChange, consignmentId, companyId, initialItems }: Props) {
  ...
  const updateMutation = useMutation({
    mutationFn: async () => {
      return ConsignmentService.updateConsignmentItems(
        consignmentId,
        companyId,
        items.map(it => ({
          id: it.id,
  ...
  const handleAddProducts = async (ids: string[]) => {
    ...
    const newItems: EditableItem[] = (products || []).map(p => ({
      product_id: p.id,
      name: p.name,
      sku: p.sku,
      sent_quantity: 1,
  ...
  const updateQty = (productId: string, qty: number) => {
    ...
    setItems(prev => prev.map(i => 
      i.product_id === productId ? { ...i, sent_quantity: Math.max(1, qty) } : i
    ));
  };
  ...
  return (
    <>
                    ...
                    <tr key={it.product_id} className="border-b border-slate-800/50 hover:bg-slate-800/20">
                      ...
                      <td className="px-4 py-3">
                        <Input 
                          ...
                          disabled={it.hasMovement}
                          onChange={(e) => updateQty(it.product_id, parseInt(e.target.value))}
                          className="h-8 w-20 mx-auto bg-slate-900 border-slate-800 text-center disabled:opacity-50"
                        />
                      </td>
      ...
      <ProductPickerDialog 
        ...
        companyId={companyId}
        excludeProductIds={new Set(items.map(i => i.product_id))}
