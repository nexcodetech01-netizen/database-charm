...
import { ArrowLeft, FileText, History, Package, User, Calendar, AlertCircle, CheckCircle2, Clock, Plus, Loader2 } from 'lucide-react';
...
export function ConsignmentDetails() {
    ...
    const updateSettlementStatusMutation = useMutation({
        mutationFn: ({ settlementId, status }) => ConsignmentService.updateSettlementStatus(settlementId, status),
        onSuccess: () => {
    ...
    return (<div className="space-y-6 pb-20">
      <div className="flex items-center gap-4">
        ...
        <div className="ml-auto flex items-center gap-2">
          <Button variant="outline" className="border-slate-800 bg-slate-900/50" onClick={handleGeneratePdf} disabled={generatingPdf}>
            {generatingPdf ? <Loader2 className="h-4 w-4 mr-2 animate-spin"/> : <FileText className="h-4 w-4 mr-2"/>}
            Contrato PDF
          </Button>
          <Button className="bg-primary hover:bg-primary/90" onClick={() => setIsSettlementOpen(true)} disabled={consignment.status === 'cancelada'}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="bg-slate-900/50 border-slate-800 md:col-span-2">
          ...
          <CardContent className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            {consignment.reseller?.document && (<div>
      ...
      <Card className="bg-slate-900/50 border-slate-800">
        ...
        <CardContent className="p-0">
          <Table>
            ...
            return (<TableRow key={item.id} className="border-slate-800 hover:bg-slate-800/30 transition-colors">
      ...
      <Card className="bg-slate-900/50 border-slate-800">
        ...
        <CardContent className="p-0">
          {isLoadingSettlements ? (<div className="p-8 text-center text-slate-500">Carregando histórico...</div>) : settlements.length === 0 ? (<div className="p-12 text-center text-slate-500 italic">
              ...
              <TableBody>
                {settlements.map((s) => (<TableRow key={s.id} className="border-slate-800 hover:bg-slate-800/30 transition-colors">
                    ...
                    <TableCell className="text-center">
                      {s.payment_status === 'pago' ? (<Badge className="bg-emerald-500/10 text-emerald-500 border-emerald-500/20 gap-1">
                          <CheckCircle2 className="h-3 w-3"/> Pago
                        </Badge>) : (<Badge className="bg-amber-500/10 text-amber-500 border-amber-500/20 gap-1">
                    ...
                    <TableCell className="text-right pr-6">
                      {s.payment_status === 'pendente' && (<Button variant="ghost" size="sm" className="text-emerald-500 hover:text-emerald-400 hover:bg-emerald-500/10" onClick={() => updateSettlementStatusMutation.mutate({ settlementId: s.id, status: 'pago' })} disabled={updateSettlementStatusMutation.isPending}>
                          Marcar como Pago
                        </Button>)}
                      {s.payment_status === 'pago' && (<Button variant="ghost" size="sm" className="text-slate-500" onClick={() => updateSettlementStatusMutation.mutate({ settlementId: s.id, status: 'pendente' })} disabled={updateSettlementStatusMutation.isPending}>
                          Estornar
                        </Button>)}
                    </TableCell>
                  </TableRow>))}
      ...
      <RegisterSettlementDialog open={isSettlementOpen} onOpenChange={setIsSettlementOpen} consignmentId={id} items={items}/>
    </div>);
}
