...
export class ConsignmentService {
    static async listResellers(companyId) {
    ...
    static async listConsignments(companyId, resellerId) {
        let query = supabase
            .from('consignacoes')
            .select(`
        *,
    ...
    static async createConsignment(consignment, items) {
        const { reseller, ...consignmentData } = consignment;
    ...
    static async registerSettlement(consignmentId, companyId, settlementData) {
        if (!companyId) {
    ...
    static async listSettlements(consignmentId) {
        const { data, error } = await supabase
            .from('consignment_settlements')
            .select('*')
    ...
    static async updateConsignmentStatus(id, status) {
    ...
    static async updateSettlementStatus(id, status) {
    ...
    static async updateConsignmentItems(consignmentId, companyId, items) {
        ...
        if (fetchError)
            throw fetchError;
        const itemsToKeepIds = items.map(it => it.id).filter(Boolean);
