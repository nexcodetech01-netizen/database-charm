import { Bell, Search, LogOut, User, Menu, Smartphone, Settings, CheckCircle, History, Check } from "lucide-react";
...
export function Topbar() {
    ...
    const { permission, requestPermission, notify, history: notificationHistory, clearHistory, markAsRead, markAllAsRead, filterType, setFilterType, filterRead, setFilterRead, page, setPage, totalPages, filteredCount } = useBrowserNotifications();
    ...
    const markAllAlertsAsRead = async () => {
        if (!companyId || activeAlerts.length === 0)
            return;
        ...
        updateCount();
        try {
            await Promise.all(alertsToRead.map(alert => {
                const payload = alert.payload;
            ...
            try {
                ...
                if (unread && unread.length > 0) {
                    ...
                    unread.forEach((notif) => {
                        const meta = BELLA_EVENT_CATALOG[notif.event_type];
                        if (!meta)
                            return;
                        const severity = notif.metadata?.severity || meta.defaultSeverity;
        ...
        const unsubscribe = bellaEventRegistry.subscribe((entry, event) => {
            if (entry.action === "created") {
                ...
                if (ticketId && notifiedIdsRef.current.has(ticketId))
                    return;
                if (ticketId)
                    notifiedIdsRef.current.add(ticketId);
    ...
    const displayName = user?.user_metadata?.full_name || user?.email || "Você";
    ...
    return (<header className="sticky top-0 z-20 flex h-16 items-center gap-3 border-b border-border bg-background/80 px-4 backdrop-blur-md sm:px-6" style={{
            paddingTop: "env(safe-area-inset-top)",
            paddingLeft: "max(1rem, env(safe-area-inset-left))",
            paddingRight: "max(1rem, env(safe-area-inset-right))",
            height: "calc(4rem + env(safe-area-inset-top))",
        }}>
      <Button type="button" variant="ghost" size="icon" className="md:hidden relative z-10 shrink-0" onClick={toggleMobileNav} aria-label="Abrir menu">
        <Menu className="h-5 w-5"/>
      </Button>
      <div className="relative min-w-0 flex-1 max-w-md">
        <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground"/>
        <input type="search" placeholder="Buscar no NexOS..." className="h-9 w-full rounded-md border border-input bg-card pl-9 pr-16 text-sm placeholder:text-muted-foreground focus:border-ring focus:outline-none focus:ring-2 focus:ring-ring/20"/>
      ...
      <div className="ml-auto flex items-center gap-1">
        ...
        <div className="flex items-center gap-1 border-r border-border pr-2 mr-1">
          {permission !== "granted" && (<Button variant="ghost" size="icon" onClick={() => requestPermission()} title="Ativar notificações do navegador" className="text-warning animate-pulse">
              <Smartphone className="h-4 w-4"/>
            </Button>)}

          <Button variant="ghost" size="icon" onClick={() => setShowSettings(true)} title="Configurações de Notificação">
        ...
        <Popover>
          <PopoverTrigger asChild>
            <div className="relative">
              <Button variant="ghost" size="icon" title="Alertas Ativos">
                ...
                {activeAlerts.length > 0 && (<Badge className="absolute -top-1 -right-1 h-4 w-4 p-0 flex items-center justify-center text-[10px] bg-primary text-primary-foreground border-2 border-background animate-in fade-in zoom-in">
          ...
          <PopoverContent className="w-80 p-0" align="end">
            <div className="flex flex-col border-b p-3 gap-2">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1">
                  <Button variant="ghost" size="icon" className="h-8 w-8" onClick={markAllAlertsAsRead} title="Resolver todos">
                    <CheckCircle className="h-3.5 w-3.5"/>
                  </Button>
                  <Button variant="ghost" size="icon" className="h-8 w-8" title="Histórico Completo" asChild>
                    <Link to="/">
            ...
            <div className="max-h-[350px] overflow-y-auto">
              {activeAlerts.length === 0 ? (<div className="p-8 text-center flex flex-col items-center gap-2">
                  <Check className="h-8 w-8 text-muted-foreground/30"/>
                  <p className="text-xs text-muted-foreground">Tudo em ordem por aqui.</p>
                </div>) : (<div className="divide-y">
                  {activeAlerts.map((alert) => (<div key={alert.id} className={cn("p-3 hover:bg-accent/50 transition-colors group relative cursor-default", alert.severity === 'critical' && "bg-destructive/5")}>
                      <div className="flex gap-3">
                        <div className={cn("mt-1 h-2 w-2 rounded-full shrink-0", alert.severity === 'critical' ? "bg-destructive" :
                    alert.severity === 'warning' ? "bg-warning" : "bg-primary")}/>
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2">
                            <Button variant="secondary" size="sm" className="h-6 px-2 text-[10px] bg-primary/10 hover:bg-primary/20 text-primary border-none" onClick={() => {
                    markAlertAsRead(alert);
                    navigate({ to: routeForEvent(alert) });
                }}>
                              Ver Detalhes
                            </Button>
                            <Button variant="ghost" size="sm" className="h-6 px-2 text-[10px] text-muted-foreground hover:text-foreground" onClick={() => markAlertAsRead(alert)}>
                              Ignorar
                            </Button>
                          </div>
                        </div>
