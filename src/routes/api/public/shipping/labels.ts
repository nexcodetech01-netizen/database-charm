...
// Force absolute fresh build v7 - Timestamp for verification
const BUILD_TS = '2026-08-17T14:43:00.000Z';
...
export const Route = createFileRoute("/api/public/shipping/labels")({
  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        const handlerVersion = 'TESTE_' + Date.now();
        console.log(`[SHIPPING_LABELS] LABELS_HANDLER_VERSION: ${handlerVersion}`);
